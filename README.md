...
Porto Release 3.3 is the text-engine refactor release. No public screenshots are embedded in this README unless they are Owner-supplied and link-checked. It introduces a DocumentBlock extraction pipeline for EPUB, PDF, DOCX and text sources; preserves semantic headings, paragraphs and list items before cleanup; keeps the Auto / Minimal / Aggressive Prepare modes; and changes MOBI/AZW handling to prefer Calibre conversion to EPUB instead of relying on the fragile legacy PalmDOC parser.
...
- Public screenshot policy: no embedded screenshots without Owner-supplied source and automated link check
...
Historical release notes remain under `docs/`. Public screenshots are not embedded unless Owner-supplied and link-checked.

## Historical technical receipts

Historical screenshot references are intentionally not embedded here; future screenshots must be Owner-supplied and link-checked before publication.
...
Historical public UI references should be added only after link-checking the exact repository path.
