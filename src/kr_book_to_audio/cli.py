...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser('ocr-run'); p.add_argument('source'); p.add_argument('--output-dir', required=True); p.add_argument('--provider')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'ocr-run':
        analysis = analyze_source(Path(args.source)); print(run_recommended_ocr(Path(args.source), analysis, output_dir=Path(args.output_dir), provider_id=args.provider)); return 0
    ...
    if args.command == 'tts':
        print(json.dumps(synthesize_parts(job, provider_id=args.engine, voice=args.voice, rate=args.rate, start=args.start, end=args.end, retries=args.retries), ensure_ascii=False, indent=2)); return 0
    if args.command == 'retry-failed':
        print(json.dumps(retry_failed_parts(job, provider_id=args.engine, voice=args.voice, rate=args.rate, retries=args.retries), ensure_ascii=False, indent=2)); return 0
    if args.command == 'preview':
        print(json.dumps(synthesize_parts(job, provider_id=args.engine, voice=args.voice, rate=args.rate, start=1, end=1, retries=args.retries, require_preview_approval=False), ensure_ascii=False, indent=2)); return 0
